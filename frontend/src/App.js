...
const state = {
  threads: [],
  token: localStorage.getItem('token'), // โหลด token จาก localStorage เมื่อเริ่ม
  error: null,
...
const API_URL = 'http://localhost:3000/api'; // URL ของ Backend
...
async function fetchThreads() {
  try {
    ...
    if (!res.ok) throw new Error('Cannot fetch threads');
...
async function handleCreateThread(e) {
  e.preventDefault();
  const title = $('#newThreadTitle').value;
  const content = $('#newThreadContent').value;
  ...
  try {
    ...
    if (!res.ok) throw new Error(newThread.message);
    
    // เพิ่มกระทู้ใหม่สำเร็จ
    state.threads.unshift(newThread); // เพิ่มกระทู้ใหม่ไปบนสุดของ list
    renderHomepage(); // render หน้า Home ใหม่
...
function renderNav() {
  ...
  if (state.token) {
    navLinks.innerHTML = `
      <li class="nav-item">
        <a class="nav-link" id="logoutBtn">ออกจากระบบ</a>
      </li>`;
...
function renderHomepage() {
  ...
  // ส่วนฟอร์มสร้างกระทู้ (แสดงเฉพาะตอน Login)
  const createThreadForm = state.token ? `
    <div class="card mb-4">
      <div class="card-body">
        <h5 class="card-title">สร้างกระทู้ใหม่</h5>
        <form id="formNewThread">
          <div class="mb-3">
            <label for="newThreadTitle" class="form-label">หัวข้อ</label>
            <input type="text" class="form-control" id="newThreadTitle" required>
          </div>
          <div class="mb-3">
            <label for="newThreadContent" class="form-label">เนื้อหา</label>
            <textarea class="form-control" id="newThreadContent" rows="3" required></textarea>
          </div>
  ...
  // ส่วนแสดงกระทู้ทั้งหมด
  const threadsList = state.threads.map(thread => `
        ...
        <div class="card-body">
            ...
            <div class="d-flex justify-content-between align-items-center">
                <small class="text-muted">
                    โพสต์โดย <strong>${thread.username}</strong>
                </small>
                <small class="text-muted">
  ...
  if (state.token) {
    $('#formNewThread').addEventListener('submit', handleCreateThread);
  }
}
